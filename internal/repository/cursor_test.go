...
func createTestCursorRepository(t *testing.T, rulesDir, promptsDir string) *repository.CursorRepository {
	repo, err := repository.NewCursorRepositoryWithPaths(rulesDir, promptsDir)
	require.NoError(t, err)
	return repo
}

func TestNewCursorRepository(t *testing.T) {
	// When creating a new repository
	repo, err := repository.NewCursorRepository()
...
func TestCursorRepository_WritePackage(t *testing.T) {
	testCases := []struct {
		name      string
		namespace string
	...
	}{
		{
			...
			validate: func(t *testing.T, rulesDir, promptsDir string) {
				...
				assert.True(t, os.IsNotExist(err), "Expected rules directory not to be created for empty package")

				_, err = os.Stat(namespacePromptsDir)
				assert.True(t, os.IsNotExist(err), "Expected prompts directory not to be created for empty package")
			},
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			repo := createTestCursorRepository(t, rulesDir, promptsDir)
...
func TestCursorRepository_WritePackage_Error(t *testing.T) {
...
func TestCursorRepository_ReadPackage(t *testing.T) {
	// Given a test repository
	tempDir := t.TempDir()
	rulesDir := filepath.Join(tempDir, "rules")
	promptsDir := filepath.Join(tempDir, "prompts")
...
func TestCursorRepository_Clean(t *testing.T) {
	...
	testRuleFile := filepath.Join(nsRulesDir, "test_rule.mdc")
	...
	assert.True(t, os.IsNotExist(err), "Rules directory should have been removed")

	_, err = os.Stat(nsPromptsDir)
	assert.True(t, os.IsNotExist(err), "Prompts directory should have been removed")
}

func TestCursorRepository_Clean_NonExistentDir(t *testing.T) {
